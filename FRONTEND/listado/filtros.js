(() => {
  ...
  const selectPlazo = document.querySelector('.filtros__select[data-filtro="plazo"]');
  ...
  GRUPOS_AUTOMATICOS.forEach((grupo) => {
    ...
    const select = document.querySelector(`.filtros__select[data-filtro="${grupo}"]`);
    select.append(new Option(NOMBRES[grupo], ""));
    valores.forEach((valor) => select.append(new Option(valor, valor)));

    const contenedor = document.querySelector(`.panel__opciones[data-grupo="${grupo}"]`);
  ...
  const etiquetaDe = (grupo, valor) => {
    ...
    if (grupo === "plazo") {
      return [...selectPlazo.options].find((opcion) => opcion.value === valor).text;
    }
    return valor;
  };
  ...
  const sincronizar = () => {
    document.querySelectorAll(".filtros__select[data-filtro]").forEach((select) => {
      select.value = elegido[select.dataset.filtro];
    });
  ...
  document.querySelectorAll(".filtros__select[data-filtro]").forEach((select) => {
    select.addEventListener("change", () => {
      elegido[select.dataset.filtro] = select.value;
      sincronizar();
    });
  });
